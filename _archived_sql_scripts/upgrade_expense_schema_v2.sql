...
ALTER TABLE expense_records ADD COLUMN moneda TEXT DEFAULT 'MXN';
...
ALTER TABLE expense_records ADD COLUMN invoice_status TEXT DEFAULT 'pending';
ALTER TABLE expense_records ADD COLUMN bank_status TEXT DEFAULT 'pending';
ALTER TABLE expense_records ADD COLUMN approval_status TEXT DEFAULT 'pending';
...
ALTER TABLE expense_records ADD COLUMN updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP;
ALTER TABLE expense_records ADD COLUMN updated_by INTEGER;
ALTER TABLE expense_records ADD COLUMN metadata TEXT; -- JSON para campos adicionales

-- 4. Crear tabla para tags (sistema de etiquetas)
...
-- 5. Crear tabla de relación many-to-many para expense_tags
...
CREATE TABLE IF NOT EXISTS expense_attachments (
    ...
    file_size INTEGER,
    mime_type TEXT,
    attachment_type TEXT DEFAULT 'receipt', -- receipt, invoice, cfdi_xml, cfdi_pdf
    uploaded_by INTEGER,
...
-- 8. Insertar tags predeterminados para el tenant principal
...
-- 10. Actualizar metadatos de schema
...
-- Este upgrade completa la coherencia entre:
-- ✅ API Models (ExpenseCreate/Response)
-- ✅ BD Schema (expense_records + related tables)
-- ✅ Business Logic (tags, attachments, approval workflow)
