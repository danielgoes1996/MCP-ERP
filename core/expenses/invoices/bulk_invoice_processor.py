...
class BulkInvoiceProcessor:
    ...
    def __init__(self):
        ...
        self.db = self._create_sync_db_wrapper()
    ...
    def _get_conn(self):
        ...
        return self.pg_adapter.connect()

    def _create_sync_db_wrapper(self):
        """Create a wrapper that provides async-like interface using sync methods"""
        class SyncDBWrapper:
            ...
            async def execute(self, query, params=None):
                """Execute query (async wrapper around sync)"""
            ...
            async def fetch_one(self, query, params=None):
                """Fetch one row (async wrapper around sync)"""
                conn = self.adapter.connect()
                try:
                    cursor = conn.cursor()
                    if params:
            ...
            async def fetch_all(self, query, params=None):
                """Fetch all rows (async wrapper around sync)"""
                conn = self.adapter.connect()
                try:
                    cursor = conn.cursor()
                    if params:
        ...
        return SyncDBWrapper(self.pg_adapter)

    async def initialize(self, db_adapter=None):
        """Inicializar el procesador con adaptador de BD (opcional)"""
        if db_adapter:
            self.db = db_adapter
    ...
    async def _process_single_item(
        ...
        try:
            ...
            # ⭐ REMOVIDO: Validación de total_amount > 0
            # Los complementos de pago (tipo P) tienen total = 0 y son válidos
            # Solo validamos que el UUID esté presente
            if not item.uuid:
                item.status = ItemStatus.ERROR
                item.error_code = "MISSING_UUID"
                item.error_message = "Missing UUID"
                return
            ...
            # Incluye tipo I (Ingreso), E (Egreso), y P (Pago)
    ...
    async def _insert_invoice_record(
        ...
        - ⭐ EXTRAE TODOS LOS CAMPOS del XML (no hardcoded)

        Returns:
            invoice_id si se insertó exitosamente, None si falló
        """
        try:
            # Get tenant_id from company
            tenant_query = "SELECT tenant_id FROM companies WHERE id = ?"
            company_data = await self.db.fetch_one(tenant_query, (batch.company_id,))

            if not company_data:
                logger.error(f"Company {batch.company_id} not found")
                return None

            tenant_id = company_data['tenant_id']

            # Si el raw_xml no está en el item, intentar leerlo del directorio test_invoices
                ...
                try:
                    ...
                    if xml_path.exists():
                        ...
                        if xml_files:
                            ...
                            logger.warning(f"XML file not found: {item.filename}")
                except Exception as e:
                    logger.warning(f"Could not load XML from file {item.filename}: {e}")

            # ⭐ NUEVO: Extraer TODOS los campos desde el XML
            ...
            fecha_timbrado = None
            regimen_fiscal_emisor = None
            version_cfdi = '4.0'

            # Parsear XML si está disponible
            if raw_xml_content:
                try:
                    ...
                    # Extraer del TimbreFiscalDigital
                    ...
                    logger.info(f"✅ Extracted all fields from XML: tipo={tipo_comprobante}, "
                              f"forma_pago={forma_pago}, metodo_pago={metodo_pago}")

                except Exception as e:
                    logger.warning(f"Could not parse XML for {item.filename}: {e}")
                    # Si falla el parsing, usar valores por defecto
                    tipo_comprobante = 'I'  # Fallback a Ingreso

            # Preparar query de inserción con TODOS los campos fiscales
            insert_query = """
            ...
            # Ejecutar inserción con datos extraídos del XML
            await self.db.execute(insert_query, (
                ...
                item.file_hash,
                item.file_size,
                # Información fiscal del emisor (proveedor)
                item.uuid,
                item.provider_rfc,
                item.provider_name,
                rfc_receptor,  # ⭐ Extraído del XML
                nombre_receptor,  # ⭐ Extraído del XML
                # Fechas
                item.issued_date,
                fecha_timbrado,  # ⭐ Extraído del XML
                # Montos
                item.subtotal_amount or 0.0,
                item.iva_amount or 0.0,
                item.total_amount,
                0.0,  # isr_retenido (TODO: extraer de retenciones)
                0.0,  # iva_retenido (TODO: extraer de retenciones)
                0.0,  # ieps_amount (TODO: extraer de traslados)
                # Información adicional
                item.currency or 'MXN',
                tipo_comprobante or 'I',  # ⭐ Extraído del XML
                forma_pago,  # ⭐ Extraído del XML
                metodo_pago,  # ⭐ Extraído del XML
                uso_cfdi,  # ⭐ Extraído del XML
                lugar_expedicion,  # ⭐ Extraído del XML
                regimen_fiscal_emisor,  # ⭐ Extraído del XML
                # Versión y status
                version_cfdi,  # ⭐ Extraído del XML
                'vigente',  # cfdi_status (TODO: validar con SAT)
                # XML completo para auditoría SAT
                raw_xml_content,
                # Status inicial
                'pending',  # status (pending hasta que se procese matching)
                datetime.utcnow()
            ))

            # Obtener el ID de la factura insertada
            # PostgreSQL no devuelve el ID automáticamente como SQLite
            # Necesitamos hacer un SELECT para obtenerlo
            id_query = """
            ...
            if result:
                ...
                logger.info(f"✅ Inserted invoice {item.filename} (tipo={tipo_comprobante}) with ID {invoice_id}")
                return invoice_id
            else:
                logger.error(f"Failed to retrieve inserted invoice ID for {item.filename}")
                return None

        except Exception as e:
            logger.error(f"Error inserting invoice {item.filename}: {e}")
            import traceback
    ...
    async def _finalize_batch(self, batch: BatchRecord):
        ...
        try:
            ...
            # Trigger webhooks for batch completion
            await self._trigger_batch_webhooks(batch)
    ...
    async def _trigger_batch_webhooks(self, batch: BatchRecord):
        """Dispara webhooks registrados para este batch"""
        try:
            import httpx

            # Get all webhooks for this batch
            query = """
                SELECT id, webhook_url FROM batch_webhooks
            ...
            if not webhooks:
                logger.debug(f"No webhooks registered for batch {batch.batch_id}")
                return

            # Prepare webhook payload
            placeholders_created = sum(
                1 for item in batch.items
                if item.status == ItemStatus.NO_MATCH and
                batch.batch_metadata.get("create_placeholder_on_no_match")
            )
            ...
            payload = {
                ...
                "success_rate": round(batch.success_rate * 100, 2) if batch.success_rate else 0,
                "processing_time_ms": batch.processing_time_ms,
                "avg_time_per_invoice_ms": batch.avg_processing_time_per_invoice,
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                for webhook_id, webhook_url in webhooks:
                    try:
                        logger.info(f"Triggering webhook {webhook_id} for batch {batch.batch_id}")
                        ...
                        """, (
                            ...
                            response.text[:1000],  # Store first 1000 chars
                            datetime.utcnow() if response.status_code < 400 else None,
                            webhook_id
                        ))
            ...
            logger.error(f"Error triggering webhooks for batch {batch.batch_id}: {e}")
        ...
        try:
            ...
            for item_data in items_data:
                item = InvoiceItem(
                    ...
                    raw_xml=None,  # Not stored in DB, would need to re-read from file if needed
            ...
            # Helper function to handle datetime conversion (PostgreSQL returns datetime objects, not strings)
            ...
            batch = BatchRecord(
                ...
                started_at=to_datetime(batch_data["started_at"]),
                completed_at=to_datetime(batch_data["completed_at"]),
                ...
                success_rate=float(batch_data["success_rate"]) if batch_data.get("success_rate") else None,
