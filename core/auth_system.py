...
class AuthService:
    ...
    def get_user_from_db(self, email: str) -> Optional[Dict]:
        """Obtener usuario de la base de datos SQLite"""
        try:
            import sqlite3
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    SELECT u.id, u.email, u.full_name, u.password_hash, u.tenant_id, u.role, u.is_active, u.created_at,
                ...
                if row:
                    ...
                    if tenant_config:
                        try:
                            config_data = json.loads(tenant_config)
