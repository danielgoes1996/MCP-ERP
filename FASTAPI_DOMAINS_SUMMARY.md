...
## Plan de Refactor por Dominio

### Invoicing Tickets
- **Archivos a crear o fusionar:**
  - Fusionar `api/advanced_invoicing_api.py`, `api/bulk_invoice_api.py`, `api/expense_completion_api.py`, `api/universal_invoice_engine_api.py` en `app/routers/invoicing_router.py`.
  - Centralizar lógica de negocio en `app/services/invoicing_service.py` y eliminar duplicados de `modules/invoicing_agent`.
- **Endpoints a mover:**
  - Mover todos los `/tickets`, `/jobs`, `/merchants` y `/v2/*` de `modules/invoicing_agent/*` hacia `/invoicing/*` en el nuevo router.
  - Unificar `/analytics` y `/rules` bajo prefijo `/invoicing/automation/*`.
- **Health checks a centralizar:**
  - Redirigir `GET /health` de `api/bulk_invoice_api.py`, `api/v1/invoicing.py` y `modules/invoicing_agent/api.py` al endpoint central `/internal/monitoring/health`.
- **Riesgos / dependencias:**
  - Validar dependencias con el frontend que consume endpoints legacy (`modules/invoicing_agent`).
  - Asegurar compatibilidad con jobs batch y workers que llaman a `/v2/*`.

### Ai Ml
- **Archivos a crear o fusionar:**
  - Crear `app/routers/ai_router.py` que agrupe `api/ai_reconciliation_api.py`, `api/category_learning_api.py` y `api/conversational_assistant_api.py`.
  - Implementar `app/services/ai_service.py` con submódulos para sugerencias, clasificación y asistente conversacional.
- **Endpoints a mover:**
  - Mover `/sessions` y `/query` del asistente a `/ai/assistant/*`.
  - Unificar `/stats` en `/ai/models/stats` diferenciando métricas de aprendizaje y reconciliación.
- **Health checks a centralizar:**
  - Centralizar `GET /health` de `api/conversational_assistant_api.py` en `/internal/monitoring/health`.
- **Riesgos / dependencias:**
  - Revisar colas o caches compartidos (Redis) antes de cambiar nombres de rutas.
  - Coordinar con equipos que consumen API de retraining (`/retrain`).

### Automation Rpa
- **Archivos a crear o fusionar:**
  - Consolidar `api/automation_v2.py`, `api/rpa_automation_engine_api.py`, `api/web_automation_engine_api.py`, `api/robust_automation_engine_api.py` en `app/routers/automation_router.py`.
  - Extraer lógica de sesiones/colas a `app/services/automation_service.py`.
- **Endpoints a mover:**
  - Migrar `/sessions/*` y `/jobs/*` legacy a nombres consistentes `/automation/sessions/*` y `/automation/jobs/*`.
  - Reemplazar variantes `robust`, `web`, `rpa`, `enhanced` por flags dentro del payload.
- **Health checks a centralizar:**
  - Canalizar `GET /health`, `GET /health/system`, `GET /analytics/{user_id}` a `/internal/monitoring/automation` con filtros por tenant.
- **Riesgos / dependencias:**
  - Verificar compatibilidad con workers y schedulers que esperan rutas concretas.
  - Actualizar configuraciones de feature flags usadas por automation v2.

### Banking Reconciliation
- **Archivos a crear o fusionar:**
  - Fusionar `api/bank_statements_api.py`, `api/payment_accounts_api.py`, `api/non_reconciliation_api.py`, `api/split_reconciliation_api.py` en `app/routers/reconciliation_router.py`.
  - Crear `app/services/reconciliation_service.py` con módulos para estados de cuenta, no conciliados y splits.
- **Endpoints a mover:**
  - Mover `/banking-institutions`, `/accounts/*`, `/records/*` bajo un esquema `/banking/*`.
  - Unificar `/suggestions` y `/splits` para exponer `/reconciliation/suggestions/*` y `/reconciliation/splits/*`.
- **Health checks a centralizar:**
  - Reubicar `GET /health/check`, `GET /health` de payment/non-reconciliation a `/internal/monitoring/finance`.
- **Riesgos / dependencias:**
  - Requiere coordinación con procesos ETL que suben estados de cuenta.
  - Confirmar reglas de permisos al mover `/mark-non-reconcilable`.

### Auth Users
- **Archivos a crear o fusionar:**
  - Fusionar `api/auth_api.py`, `api/auth_jwt_api.py` y `api/v1/user_context.py` en `app/routers/auth_router.py`.
  - Crear `app/services/auth_service.py` con soporte JWT + contexto de usuario.
- **Endpoints a mover:**
  - Exponer `/auth/login`, `/auth/logout`, `/auth/refresh`, `/auth/me` y mover `/context/*` a `/auth/context/*`.
  - Eliminar duplicados `/login` y `/logout` legacy tras migración.
- **Health checks a centralizar:**
  - Remover `GET /health` y `GET /info` del router de auth; delegar a `/internal/monitoring/auth`.
- **Riesgos / dependencias:**
  - Reevaluar expiración de tokens al tocar múltiples controladores.
  - Coordinar migración con el frontend de onboarding que usa endpoints legacy.

### Financial Reports Insights
- **Archivos a crear o fusionar:**
  - Integrar `api/financial_reports_api.py`, `api/financial_intelligence_api.py`, `api/employee_advances_api.py` en `app/routers/finance_router.py`.
  - Generar `app/services/finance_service.py` con capas para reportes, inteligencia y anticipos.
- **Endpoints a mover:**
  - Mover `/poliza-electronica/*` y `/iva` hacia `/finance/reports/*`.
  - Deprecar `/v1/polizas_api.py` creando rutas equivalentes en el nuevo router.
- **Health checks a centralizar:**
  - Centralizar `GET /health` de `api/financial_reports_api.py` en `/internal/monitoring/finance`.
- **Riesgos / dependencias:**
  - Validar integraciones con SAT y descargas XML al cambiar rutas.
  - Confirmar migración de datos de employee advances antes de deshabilitar rutas legacy.

### Feature Flags Config
- **Archivos a crear o fusionar:**
  - Mantener `api/feature_flags_api.py` pero moverlo a `app/routers/config_router.py`.
  - Crear `app/services/config_service.py` para encapsular acceso a `automation_config`.
- **Endpoints a mover:**
  - Renombrar `/automation/*` a `/config/automation/*` y `/check/{flag_key}` a `/config/flags/{flag_key}`.
  - Publicar un único endpoint `/config/flags` que combine estado y auditoría.
- **Health checks a centralizar:**
  - Sin health dedicado; agregar métricas en `/internal/monitoring/config` si se requieren.
- **Riesgos / dependencias:**
  - Revisar dependencias con servicios que leen directamente `automation_config`.
  - Asegurar migración de scopes (global/company/user) en respuestas.

### Clients Companies
- **Archivos a crear o fusionar:**
  - Agrupar `api/client_management_api.py` y `api/v1/companies_context.py` en `app/routers/clients_router.py`.
  - Crear `app/services/clients_service.py` para lógica de setup y contexto.
- **Endpoints a mover:**
  - Mover `/context/*` bajo `/clients/context/*`.
  - Normalizar `/batch-setup`, `/setup` y `/portals` dentro de `/clients/*`.
- **Health checks a centralizar:**
  - Mover `GET /health` de client management a `/internal/monitoring/clients`.
- **Riesgos / dependencias:**
  - Verificar dependencia con pipelines de onboarding que consumen `/contextual_profile`.
  - Alinear permisos multi-tenant tras la fusión.

### Debug Health
- **Archivos a crear o fusionar:**
  - Conservar `api/v1/debug.py` pero reubicarlo como `app/routers/debug_router.py`.
  - Implementar `app/services/debug_service.py` para encapsular telemetría.
- **Endpoints a mover:**
  - Consolidar todos los `GET /health` externos en `/internal/monitoring/health` y `/internal/monitoring/automation`.
  - Agregar endpoints de métricas internas bajo `/internal/monitoring/*`.
- **Health checks a centralizar:**
  - Este dominio será el hub de health/monitoring, eliminando health checks dispersos.
- **Riesgos / dependencias:**
  - Revisar políticas de autenticación para endpoints internos.
  - Garantizar que el sistema de alertas consume las nuevas rutas centralizadas.

## Checklist de implementación
- [ ] Inventariar consumidores externos e internos de cada endpoint legacy.
- [ ] Crear routers y servicios base (`app/routers/*`, `app/services/*`) sin eliminar endpoints antiguos.
- [ ] Reimplementar endpoints core en los nuevos routers y cubrir con pruebas/regresión.
- [ ] Redirigir tráfico (reverse proxy o capa de compatibilidad) hacia las rutas nuevas.
- [ ] Eliminar endpoints legacy y módulos duplicados una vez verificada la adopción.
- [ ] Actualizar documentación y clientes SDK con los paths consolidados.
- [ ] Ejecutar smoke tests sobre `/internal/monitoring/*` y validar alertas.